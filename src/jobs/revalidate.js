...
export function passColumns(row, result) {
  if (result.feedUnchanged) {
    return {
      ...
      // The one field a 304 *can* still move: the feed URL isn't read from the body,
      // it's where the fetch landed, so a feed that has since grown a permanent
      // redirect gets normalised here rather than waiting for its next real change.
