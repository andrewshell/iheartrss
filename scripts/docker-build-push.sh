...
# Docker Build and Push Script for iheartrss
...
# Configuration
DOCKER_REPO="ghcr.io/andrewshell/iheartrss"
DOCKERFILE_PATH="Dockerfile"
VERSION_PACKAGE_JSON="package.json"
...
# There is no typecheck and no build step: plain ESM, no TypeScript, no bundler.
run_quality_checks() {
    ...
    if ! pnpm format:check; then
        log_error "Formatting check failed (run 'pnpm format')"
        exit 1
    fi
...
show_image_info() {
    ...
    echo "   # ./data must exist and be owned by uid 1000 (the container's \`node\` user)"
    echo "   # ./secrets/ip_hmac_key must exist: head -c 32 /dev/urandom | base64 > secrets/ip_hmac_key"
    echo "   docker run -d -p 127.0.0.1:3000:3000 \\"
    echo "     -e SITE_URL=https://iheartrss.com \\"
    echo "     -e DATABASE_PATH=/data/iheartrss.db \\"
    echo "     -e IP_HMAC_KEY_FILE=/run/secrets/ip_hmac_key \\"
    echo "     -e TRUST_PROXY=true -e TRUSTED_PROXY_HOPS=0 \\"
    echo "     -e ADMIN_TOKEN=... \\"
    echo "     -v \"\$PWD/data:/data\" \\"
    echo "     -v \"\$PWD/content:/app/content:ro\" \\"
    echo "     -v \"\$PWD/secrets/ip_hmac_key:/run/secrets/ip_hmac_key:ro\" \\"
    echo "     ${DOCKER_REPO}:latest"
    echo ""
    echo "   (In production this is docker-compose.yml under dockge, not a bare"
    echo "    docker run — see README.md, \"Deploying\".)"
    echo ""
}

# Main execution
main() {
    echo "🐳 iheartrss Docker Build & Push Script"
    echo "======================================="
    echo ""
